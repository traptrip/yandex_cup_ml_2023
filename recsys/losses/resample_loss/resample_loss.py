...
class ResampleLoss(nn.Module):
    def __init__(
        ...
        with open(DATA_DIR / "class_freq.pkl", "rb") as f:
            clsfreq = pickle.load(f)

        self.class_freq = (
            torch.from_numpy(np.asarray(clsfreq["class_freq"])).float().to(self.device)
        )
        self.neg_class_freq = (
            torch.from_numpy(np.asarray(clsfreq["neg_class_freq"]))
            .float()
            .to(self.device)
        )
